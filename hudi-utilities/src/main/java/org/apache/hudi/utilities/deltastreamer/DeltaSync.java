...
public class DeltaSync implements Serializable {
    ...
    if (!hasErrors || cfg.commitOnErrors) {
      ...
      writeStatusRDD.filter(ws -> ws.hasErrors()).take(100).forEach(ws -> {
        ...
        if (ws.getErrors().size() > 0) {
          ws.getErrors().entrySet().forEach(r -> LOG.trace("Error for key:{} is {}",r.getKey(), r.getValue()));
        }
      });
  ...
  private void syncHive() throws ClassNotFoundException {
    if (cfg.enableHiveSync) {
      ...
      LOG.info("Syncing target hoodie table with hive table({}). Hive metastore URL :{}, basePath :{}",hiveSyncConfig.tableName,hiveSyncConfig.jdbcUrl, cfg.targetBasePath);
